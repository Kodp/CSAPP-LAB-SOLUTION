...
static int add_range(range_t** ranges, char* lo, int size, int tracenum, int opnum) {
    ...
    if ((lo < (char*)mem_heap_lo()) || (lo > (char*)mem_heap_hi()) || (hi < (char*)mem_heap_lo()) ||
        (hi > (char*)mem_heap_hi())) {
        sprintf(msg, "Payload (%p:%p) lies outside heap (%p:%p)", lo, hi, mem_heap_lo(),
                mem_heap_hi());
        malloc_error(tracenum, opnum, msg);
        return 0;
    }
...
static trace_t* read_trace(char* tracedir, char* filename) {
    ...
    __ret = fscanf(tracefile, "%d", &(trace->weight)); /* not used */
    ...
    while (fscanf(tracefile, "%s", type) != EOF) {
        switch (type[0]) {
        case 'a':
            __ret = fscanf(tracefile, "%u %u", &index, &size);
        ...
        case 'r':
            ...
            break;
        case 'f':
            __ret = fscanf(tracefile, "%ud", &index);
...
static double eval_mm_util(trace_t* trace, int tracenum, range_t** ranges) {
    ...
    printf("max total size: %d, heap size: %d\n", max_total_size, mem_heapsize());
