...
instr_t instruction_set[] = 
{
	...
	// {"gaddq",  HPACK(I_GADDQ, C_G), 10, I_ARG, 2, 8, R_ARG, 1, 0 },
	{"caddtq",  HPACK(I_CADDTQ, F_NONE), 10, I_ARG, 2, 8, R_ARG, 1, 0 },
	{"shlq",  HPACK(I_SHLQ, A_SHL), 10, I_ARG, 2, 8, R_ARG, 1, 0 },
...
cc_t compute_cc(alu_t op, word_t argA, word_t argB)
{
	...
	case A_SHL:
	// 带符号左移溢出：
    // 如果结果 val 的符号与原始被移位数值 argB 的符号不同，则发生溢出。
    // 这涵盖了 argB 是正数左移变为负数，或 argB 是负数左移变为正数的情况。
    ovf = (((word_t) val < 0) != ((word_t) argB < 0));
	break;
    case A_AND:
    case A_XOR:
	ovf = FALSE;
...
stat_t step_state(state_ptr s, FILE *error_file)
{
	...
	(hi0 == I_RRMOVQ || hi0 == I_ALU || hi0 == I_PUSHQ ||
	 ...
	 hi0 == I_MRMOVQ || hi0 == I_IADDQ || hi0 == I_CADDTQ || hi0 == I_SHLQ);
	...
	(hi0 == I_IRMOVQ || hi0 == I_RMMOVQ || hi0 == I_MRMOVQ ||
	 hi0 == I_JMP || hi0 == I_CALL || hi0 == I_IADDQ || hi0 == I_CADDTQ || hi0 == I_SHLQ);
	...
	// 	argB = get_reg_val(s->r, lo1);
	// 	/**
	// 	 * lo0是byte0的低4位，byte0就是指令的第一个字节，其低4位是ifun;
	// 	 * 上面添加 gaddq时，我们将 (I_GADDQ, C_G) 打包到一起，于是lo0=C_G（大于判断）;
	// 	 * cond_holds会读取cc和C_G，判断lo0是否是大于的条件。
	// 	 * 如果返回True，则说明大于，于是我们就执行加法，否则不执行。
	// 	*/
	// 	if (cond_holds(s->cc, lo0)) {
	// 		val = argB + cval;
	// 	}
	...
	case I_CADDTQ:
		...
		argB = get_reg_val(s->r, lo1);
		word_t rax = get_reg_val(s->r, REG_RAX);
		// val = argB + cval;  
		// set_reg_val(s->r, lo1, val);
		val = rax + (argB > 0 ? cval : 0);
		set_reg_val(s->r, REG_RAX, val);
		// fprintf(stdout, "fuckyou!");

		// s->cc = compute_cc(A_ADD, cval, argB); // 我们hcl的设计中不设置cond
		s->pc = ftpc;
		break;
	...
	case I_SHLQ:
		...
		val = argB << cval;  
		set_reg_val(s->r, lo1, val);
		s->cc = compute_cc(A_SHL, cval, argB);  // 我们hcl的设计中不设置cond
		s->pc = ftpc;
		break;
    default:
	if (error_file)
