 ...
 **************************************************************************/ 
#define _POSIX_C_SOURCE 200809L  /* 启用posix标准，防止报错 */
#include <stdio.h>
#include <stdlib.h>
#include <stdarg.h>
#include <unistd.h>
...
void tty_report(word_t cyc) {
  ...
  sim_log("M: instr = %s, Cnd = %d, valE = 0x%llx, valA = 0x%llx\n   dstE = %s, dstM = %s, Stat = %s, **m_valM = 0x%llx\n",
	  ...
	  stat_name(ex_mem_curr->status), mem_wb_curr->valm);
...
static byte_t sim_step_pipe(word_t max_instr, word_t ccount)
{
    ...
    // 新顺序为mem，ex，if，这样if阶段就能吃到mem、ex的修改。
    do_mem_stage();  // mem必须在ex之前执行  // why?
    do_ex_stage();
    do_if_stage();

    do_id_wb_stages(); // decode必须在ex和mem之后执行

    // 模拟器只需要执行CPU相同的效果即可，不需要完全按照IF,ID,EX,MEM,WB的顺序执行;
    // 只需要保证实现等价于处理器的功能即可。和CPU设计只需要实现ISA的表象，硬件层面无须完全一致有异曲同工之妙。
