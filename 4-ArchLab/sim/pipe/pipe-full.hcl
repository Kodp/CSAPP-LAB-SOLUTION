...
# wordsig IGADDQ  'I_GADDQ'  # gaddq 被删除，但为了保留其hcl实现，将其先伪造为NOP通过编译
wordsig IGADDQ  'I_NOP'
wordsig ICADDTQ 'I_CADDTQ'  # misc内添加 Conditional add template 指令
wordsig ISHLQ   'I_SHLQ'
# template指的是，添加的目的寄存器在hcl里修改；默认为%rax。
...
wordsig RRAX	 'REG_RAX'    # for caddtq

##### ALU Functions referenced explicitly ##########################
wordsig ALUADD	'A_ADD'		     # ALU should add its arguments
wordsig ALUSHL  'A_SHL'
...
## 确定下一条指令的取指地址
word f_pc = [
    # 分支预测错误：如果 E 阶段的指令是条件跳转 (IJXX) 且条件不满足 (预测错误)，
    # # 则取指地址为 E 阶段计算出的正确目标地址,即下一条地址 (E_valA)
	# M_icode == IJXX && !M_Cnd : M_valA;
	E_icode == IJXX && !e_Cnd : E_valA;
	# RET 指令完成：如果 M 阶段的指令是 RET，则取弹出的地址; 
	M_icode == IRET : m_valM;
	# 默认情况：使用 F 阶段预测的下一条指令地址 (F_predPC)。
	1 : F_predPC;
];
...
word d_srcA = [
	...
	D_icode == ICADDTQ: RRAX; 			# caddtq使用 RAX 作为源寄存器
	1 : RNONE; # Don't need register
];

## What register should be used as the B source?
word d_srcB = [
	D_icode in { IOPQ, IRMMOVQ, IMRMOVQ, IIADDQ,
			     IGADDQ, ICADDTQ, ISHLQ } : D_rB;
...
word d_dstE = [
	D_icode in { IRRMOVQ, IIRMOVQ, IOPQ, IIADDQ, 
		         IGADDQ, ISHLQ } : D_rB;
	D_icode in { IPUSHQ, IPOPQ, ICALL, IRET } : RRSP;
	D_icode == ICADDTQ: RRAX;      		# 使用 RAX 作为目的寄存器
...
word aluA = [
	...
	E_icode in { IIRMOVQ, IRMMOVQ, IMRMOVQ, IIADDQ, 
		         IGADDQ, ISHLQ } : E_valC;
	E_icode == ICADDTQ && E_valB > 0: E_valC;
	E_icode == ICADDTQ && !(E_valB > 0) : 0; # 如果 E_valB <= 0, 则不需要加数
...
word aluB = [
	E_icode in { IRMMOVQ, IMRMOVQ, IOPQ, ICALL, 
		     IPUSHQ, IRET, IPOPQ, IIADDQ,
			 	 IGADDQ, ISHLQ } : E_valB;
	E_icode in { IRRMOVQ, IIRMOVQ } : 0;
	E_icode == ICADDTQ: E_valA;  # R[%rax]
	# Other instructions don't need ALU
];
...
word e_valA = [
	E_icode in {IPUSHQ, IRMMOVQ} && M_dstM == E_srcA: m_valM; 
	1: E_valA;
]; # 修改 e_valA: （接受）转发，拿到m_valM的值。

## Set dstE to RNONE in event of not-taken conditional move
word e_dstE = [
	E_icode == IRRMOVQ && !e_Cnd : RNONE;
	E_icode == IGADDQ && !e_Cnd : RNONE;
	1 : E_dstE;
];
...
# load/use hazard || ret stall
# 当ret指令在D或E阶段 且 E阶段不是分支预测错误的情况 (E阶段分支预测错误无须stall,转发解决问题)
bool F_stall = 
    (E_icode in { IMRMOVQ, IPOPQ } && (E_dstM == d_srcB || E_dstM == d_srcA && !D_icode in { IRMMOVQ, IPUSHQ})) 
    || (IRET in { D_icode, E_icode } && !(E_icode == IJXX && !e_Cnd));
	# 让 ret 的 stall 和 bubble 排除自己 (ret) 是错误分支预测的情况，错误分支就不要 F_stall 和 D_bubble。

# Should I stall or inject a bubble into Pipeline Register D?
# At most one of these can be true.
bool D_stall = # load/use hazard
	E_icode in { IMRMOVQ, IPOPQ } && (E_dstM == d_srcB || E_dstM == d_srcA && !D_icode in { IRMMOVQ, IPUSHQ});

bool D_bubble = 
    # 去除了之前的“错误跳转判断”。
    # 起泡在：
    # 1. 没有load/use​（load/use需要D_stall,E_bubble）
    # ​​2. 当ret指令在D或E阶段 且 E阶段不是分支预测错误的情况
    !(E_icode in { IMRMOVQ, IPOPQ } && E_dstM in { d_srcA, d_srcB }) 
    && (IRET in { D_icode, E_icode } && !(E_icode == IJXX && !e_Cnd));
	# 让 ret 的 stall 和 bubble 排除自己 (ret) 是错误分支预测的情况，错误分支就不要 F_stall 和 D_bubble。由jXX转发处理F阶段、E_bubble清除ret。
